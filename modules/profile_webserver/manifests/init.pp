class profile_webserver (
	...
	$site_name	= "demo",
	$port		= "80",
	$python_mods	= [ 'flask', 'flask-sqlalchemy', 'sqlalchemy' ],
	...
	apache::vhost { "${site_name}":
		...
		wsgi_daemon_process_options	=> { threads => 5, },
		wsgi_application_group		=> '%{GLOBAL}',
		wsgi_process_group		=> "${site_name}",		
		wsgi_script_aliases		=> { '/' => "/var/www/${site_name}/${site_name}.wsgi" },
		require				=> Class['apache::mod::wsgi'],
		before				=> File["/var/www/${site_name}"],
	}
